	...
	{                                                                                                        \
		...
		{                                                                                                    \
			...
			{                                                                                                \
				...
				memcpy(SCREEN + (y * 2 + 1) * GFX.RealPitch, SCREEN + y * 2 * GFX.RealPitch, GFX.RealPitch); \
				                                                                                             \
				if (DO_DEPTH)                                                                                \
				{                                                                                            \
...
void StartScreenRefresh()
{
	...
	if (++IPPU.FrameCount == (Settings.PAL ? 50 : 60))
		IPPU.FrameCount = 0;
}
...
void RenderLine(uint8_t C)
{
	if (!IPPU.RenderThisFrame)
	{
		if (IPPU.OBJChanged) /* We still need to update this */
			SetupOBJ();
...
void SetupOBJ()
{
	uint8_t S, FirstSprite;
	int32_t Height, SmallWidth, SmallHeight, LargeWidth, LargeHeight;
		...
		do
		{
			...
			if (HPos > -GFX.OBJWidths[S] && HPos <= 256)
			{
				...
				for (line = 0, Y = (uint8_t) (PPU.OBJ[S].VPos & 0xff); line < Height; Y++, line++)
				{
					...
					if (PPU.OBJ[S].VFlip) /* Yes, Width not Height. It so happens that the sprites with H = 2 * W flip as two W * W sprites. */
	...
	else /* evil FirstSprite+Y case */
	{
		...
		/* We only initialise this per line, as needed. [Neb] - Bonus: We can quickly avoid looping if a line has no OBJs. */
		bool AnyOBJOnLine[SNES_HEIGHT_EXTENDED];
		memset(AnyOBJOnLine, false, sizeof(AnyOBJOnLine));

		for (S = 0; S < 128; S++) /* First, find out which sprites are on which lines */
		{
			int32_t HPos;
			...
			if (HPos > -GFX.OBJWidths[S] && HPos <= 256)
			{
				...
				for (line = 0, Y = (uint8_t) (PPU.OBJ[S].VPos & 0xff); line < Height; Y++, line++)
				{
					...
					if (PPU.OBJ[S].VFlip) /* Yes, Width not Height. It so happens that the sprites with H=2*W flip as two WxW sprites. */
		...
		for (Y = 0; Y < SNES_HEIGHT_EXTENDED; Y++) /* Now go through and pull out those OBJ that are actually visible. */
		{
			...
			if (AnyOBJOnLine[Y])
			{
				FirstSprite = (PPU.FirstSprite + Y) & 0x7F;
				S = FirstSprite;
						...
						{                                                                                               \
							...
							{                                                                                           \
								...
								area4        = tmp;                                                                     \
							                                                                                            ...
							                                                                                            \
							tempColor = ((area1 * p1) + (area2 * p2) + (area3 * p3) + (area4 * p4)) >> 5;               \
...
void UpdateScreen()
{
	...
	if (PPU.BGMode == 5 || PPU.BGMode == 6 || IPPU.Interlace || IPPU.DoubleHeightPixels)
	{
		...
		if ((PPU.BGMode == 5 || PPU.BGMode == 6) && !IPPU.DoubleWidthPixels) /* The game has switched from lo-res to hi-res mode part way down the screen. Scale any existing lo-res pixels on screen */
		{
			uint32_t y;
			...
			{
				int32_t   x;
				uint16_t* p = (uint16_t*) (GFX.Screen + y * GFX.RealPitch) + 255;
		...
		if (IPPU.Interlace && !IPPU.DoubleHeightPixels) /* BJ: And we have to change the height if Interlace gets set, too. */
		{
			int32_t y;
			starty                    = GFX.StartY * 2;
			endy                      = GFX.EndY * 2 + 1;
			...
			/* The game has switched from non-interlaced to interlaced mode part way down the screen. Scale everything. */
			for (y = (int32_t) GFX.StartY - 1; y >= 0; y--)
			{
				/* memmove converted: Same malloc, different addresses, and identical addresses at line 0 [Neb] */
				memcpy(GFX.Screen + y * 2 * GFX.RealPitch, GFX.Screen + y * GFX.RealPitch, GFX.RealPitch);
				/* memmove converted: Same malloc, different addresses [Neb] */
				memcpy(GFX.Screen + (y * 2 + 1) * GFX.RealPitch, GFX.Screen + y * GFX.RealPitch, GFX.RealPitch);
			}
		}
	}
	...
	if (!PPU.ForcedBlanking && ADD_OR_SUB_ON_ANYTHING && (GFX.r2130 & 0x30) != 0x30 && !((GFX.r2130 & 0x30) == 0x10 && IPPU.Clip[1].Count[5] == 0))
	{
		...
		if (pClip->Count[5]) /* Colour window enabled. */
		{
			uint32_t y;
			...
			for (y = starty; y <= endy; y++)
			{
				...
				{
					uint32_t* p = (uint32_t*) (GFX.SubScreen + y * GFX.RealPitch);
				...
				for (c = 0; c < pClip->Count[5]; c++)
				{
					if (pClip->Right[c][5] > pClip->Left[c][5])
					{
						...
						if (IPPU.Clip[0].Count[5])
						{
							/* Blast, have to clear the sub-screen to the fixed-colour because there is a colour window in effect clipping
							 * the main screen that will allow the sub-screen 'underneath' to show through. */
							uint16_t* p = (uint16_t*) (GFX.SubScreen + y * GFX.RealPitch);
		...
		if (IPPU.Clip[0].Count[5])
		{
			...
			for (y = starty; y <= endy; y++)
			{
				uint16_t* p = (uint16_t*) (GFX.Screen + y * GFX.RealPitch);
				...
				for (b = 0; b < Count; b++)
				{
					...
					if (GFX.r2131 & 0x80)
					{
						if (GFX.r2131 & 0x40)
						{
							...
							uint16_t* p          = (uint16_t*) (GFX.Screen + y * GFX.RealPitch) + Left;
					...
					else if (!pClip->Count[5])
					{
						/* The backdrop has not been cleared yet - so copy the sub-screen to the main screen
						 * or fill it with the back-drop colour if the sub-screen is clear. */
						uint16_t* p = (uint16_t*) (GFX.Screen + y * GFX.RealPitch) + Left;
						uint8_t*  d = GFX.ZBuffer + y * GFX.ZPitch;
						uint8_t*  s = GFX.SubZBuffer + y * GFX.ZPitch + Left;
						uint8_t*  e = d + Right;
		...
		{
			uint32_t y;
			uint32_t back = IPPU.ScreenColors[0] | (IPPU.ScreenColors[0] << 16); /* Subscreen not being added to back */
			pClip = &IPPU.Clip[0];
	...
	else /* 16bit and transparency but currently no transparency effects in operation. */
	{
		uint32_t y, back = IPPU.ScreenColors[0] | (IPPU.ScreenColors[0] << 16);
		...
		if (IPPU.Clip[0].Count[5])
		{
			...
			{
				uint32_t  c;
				uint32_t* p = (uint32_t*) (GFX.Screen + y * GFX.RealPitch);
				uint32_t* q = (uint32_t*) ((uint16_t*) p + IPPU.RenderedScreenWidth);
	...
	if (PPU.BGMode != 5 && PPU.BGMode != 6 && IPPU.DoubleWidthPixels) /* Mixture of background modes used on screen - scale width of all non-mode 5 and 6 pixels. */
	{
		uint32_t y;
