...
static const int16_t ST010_M7Scale[176] = /* Mode 7 scaling constants for all raster lines */
{
...
{
	*X1 = (Y0 * math_sin(Theta) >> 15) + (X0 * math_cos(Theta) >> 15);
	*Y1 = (Y0 * math_cos(Theta) >> 15) - (X0 * math_sin(Theta) >> 15);
}

void ST010_SortDrivers(uint16_t Positions, uint16_t* Places, uint16_t* Drivers)
{
	bool Sorted;
	uint16_t Temp;
...
#define ST010_WORD(offset) \
	(Memory.SRAM[offset + 1] << 8) | Memory.SRAM[offset]
...
void SetST010(uint8_t Byte, uint32_t Address)
{
	...
	switch (ST010.op_reg)
	{
		...
		case 0x07:
		{
			...
			for (line = 0; line < 176; line++, offset += 2)
			{
				/* Calculate Mode 7 Matrix A/D data */
				data = ST010_M7Scale[line] * math_cos(Theta) >> 15;
				...
				data = ST010_M7Scale[line] * math_sin(Theta) >> 15;
		...
		case 0x04: /* calculate the vector length of (x,y) */
		{
			int16_t square, x, y;
		...
		case 0x05: /* calculate AI orientation based on specific guidelines */
		{
			int32_t dx, dy;
			int16_t a1, b1, c1;
			...
			if (MATH_ABS(o1 - rot) > 0x8000) /* check for wrapping */
			{
				o1 += 0x8000;
				rot += 0x8000;
				wrap = true;
			...
			if (MATH_ABS(o1 - rot) == 0x8000) /* special case */
				speed = 0x100;
			else if (MATH_ABS(o1 - rot) >= 0x1000) /* slow down for sharp curves */
			{
				uint32_t slow = MATH_ABS(o1 - rot);
				slow >>= 4; /* scaling */
				speed -= slow;
			}
			...
			if (MATH_ABS(old_speed - speed) > 0x8000) /* prevent negative/positive overflow */
			{
				if (old_speed < speed)
					speed = 0;
				else
			...
			if ((o1 > rot && (o1 - rot) > 0x80) || (o1 < rot && (rot - o1) >= 0x80)) /* adjust direction by so many degrees; be careful of negative adjustments */
			{
				if (o1 < rot)
					rot -= 0x280;
				else if (o1 > rot)
			...
			if ((system && (dy <= 6 && dy >= -8) && (dx <= 126 && dx >= -128)) || (!system && (dx <= 6 && dx >= -8) && (dy <= 126 && dy >= -128))) /* if we're in so many units of the target, signal it */
			{
			...
			/* update position */
			xpos -= (math_cos(rot) * 0x400 >> 15) * (speed >> 8) << 1;
			ypos -= (math_sin(rot) * 0x400 >> 15) * (speed >> 8) << 1;
