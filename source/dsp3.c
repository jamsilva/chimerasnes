...
static bool DSP3_GetBits(uint8_t);
static void DSP3_TestMemory();
static void DSP3_DumpDataROM();
static void DSP3_MemoryDump();
static void DSP3_Coordinate();
static void DSP3_Command();
static void DSP3_Decode_Data();
static void DSP3_Decode_Tree();
static void DSP3_Decode_Symbols();
static void DSP3_Decode();
static void DSP3_Decode_A();
static void DSP3_Convert();
static void DSP3_Convert_A();
static void DSP3_OP03();
static void DSP3_OP06();
static void DSP3_OP07();
static void DSP3_OP07_A();
static void DSP3_OP07_B();
static void DSP3_OP0C();
static void DSP3_OP10();
static void DSP3_OP1C();
static void DSP3_OP1C_A();
static void DSP3_OP1C_B();
static void DSP3_OP1C_C();
static void DSP3_OP1E();
static void DSP3_OP1E_A();
static void DSP3_OP1E_A1();
static void DSP3_OP1E_A2();
static void DSP3_OP1E_A3();
static void DSP3_OP1E_B();
static void DSP3_OP1E_B1();
static void DSP3_OP1E_B2();
static void DSP3_OP1E_C();
static void DSP3_OP1E_C1();
static void DSP3_OP1E_C2();
static void DSP3_OP1E_D(int16_t, int16_t*, int16_t*);
static void DSP3_OP1E_D1(int16_t, int16_t*, int16_t*);
static void DSP3_OP3E();
...
static void DSP3_OP03()
{
	int16_t Lo = (uint8_t)  DSP3.DR;
	int16_t Hi = (uint8_t) (DSP3.DR >> 8);
	int16_t Ofs = (DSP3.WinLo * Hi << 1) + (Lo << 1);
	DSP3.DR = Ofs >> 1;
	SetDSP3 = &DSP3_Reset;
}
...
static void DSP3_OP07_B()
{
	int16_t Ofs = (DSP3.WinLo * DSP3.AddHi << 1) + (DSP3.AddLo << 1);
	DSP3.DR = Ofs >> 1;
	SetDSP3 = &DSP3_Reset;
}
...
static void DSP3_OP07()
{
	uint32_t dataOfs = ((DSP3.DR << 1) + 0x03b2) & 0x03ff;
...
/* Opcodes 1E/3E bit-perfect to 'dsp3-intro' log, source adapted from SD Gundam X/G-Next */
static void DSP3_OP3E()
{
...
static void DSP3_OP1E()
{
	...
	for (int32_t lcv = 0; lcv < DSP3.op1e_min_radius; lcv++)
...
static void DSP3_OP1E_A()
{
	if (DSP3.op1e_lcv_steps == 0)
	{
		...
		for (int32_t lcv = 0; lcv < DSP3.op1e_lcv_radius; lcv++)
			DSP3_OP1E_D(DSP3.op1e_turn, &DSP3.op1e_x, &DSP3.op1e_y);
	}
...
static void DSP3_OP1E_A3()
{
	...
	else if (DSP3.op1e_terrain[DSP3.op1e_cell] & 1)
		DSP3.op1e_weight[DSP3.op1e_cell] = 0xff;
	else
...
static void DSP3_OP1E_B2()
{
	...
	while (lcv_turns)
	{
		...
		if (y >= 0 && y < DSP3.WinHi && x >= 0 && x < DSP3.WinLo)
...
static void DSP3_OP1E_D(int16_t move, int16_t* lo, int16_t* hi)
{
	...
	int16_t Lo = (uint8_t) *lo;
	int16_t Hi = (uint8_t) *hi;
	DSP3.AddHi = DSP3_DataROM[dataOfs];
	DSP3.AddLo = DSP3_DataROM[dataOfs + 1];
...
{
	const uint16_t HiAdd[] =
	{
		0x00, 0xFF, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00,
		0x00, 0xFF, 0xFF, 0x00, 0x01, 0x00, 0xFF, 0x00
	};

	const uint16_t LoAdd[] = {0x00, 0x00, 0x01, 0x01, 0x00, 0xFF, 0xFF, 0x00};
...
static void DSP3_OP0C() /* absorb 2 bytes */
{
	DSP3.DR = 0;
	SetDSP3 = &DSP3_Reset;
}

static void DSP3_OP1C_C() /* return 2 bytes */
{
	DSP3.DR = 0;
	SetDSP3 = &DSP3_Reset;
}

static void DSP3_OP1C_B() /* return 2 bytes */
{
	DSP3.DR = 0;
	SetDSP3 = &DSP3_OP1C_C;
}

static void DSP3_OP1C_A() /* absorb 2 bytes */
{
	SetDSP3 = &DSP3_OP1C_B;
}

static void DSP3_OP1C() /* absorb 2 bytes */
{
	SetDSP3 = &DSP3_OP1C_A;
}
