...
/* Arc-tangent extracted from c4.cpp */
int16_t math_atan2(int16_t x, int16_t y)
{
	...
	if (x1 > y1)
		absAtan = AtanTable[(uint8_t) ((y1 << 8) / x1)];
	else
		absAtan = AtanTable[(uint8_t) ((x1 << 8) / y1)];
...
/* Cosine and sine extracted from dsp1.cpp */
int16_t math_cos(int16_t angle)
{
	...
	angleS8 = angle >> 8;
	S = SinTable[0x40 + angleS8] - (MulTable[angle & 0xff] * SinTable[angleS8] >> 15);
...
int16_t math_sin(int16_t angle)
{
	...
	angleS8 = angle >> 8;
	S = SinTable[angleS8] + (MulTable[angle & 0xff] * SinTable[0x40 + angleS8] >> 15);
