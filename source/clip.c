...
#define BANDS_INTERSECT(A, B) \
	((A.Left >= B.Left && A.Left < B.Right) || (B.Left >= A.Left && B.Left < A.Right))

#define OR_BANDS(R, A, B)                     \
	{                                         \
		R.Left  = MATH_MIN(A.Left, B.Left);   \
		R.Right = MATH_MAX(A.Right, B.Right); \
	}

#define AND_BANDS(R, A, B)                    \
	{                                         \
		R.Left  = MATH_MAX(A.Left, B.Left);   \
		R.Right = MATH_MIN(A.Right, B.Right); \
	}
...
void ComputeClipWindows()
{
	...
	for (c = 0; c < 2; c++, pClip++) /* Loop around the main screen then the sub-screen. */
	{
			...
			if (w == 5 || pClip->Count[5] || (Memory.FillRAM[0x212c + c] & Memory.FillRAM[0x212e + c] & (1 << w)))
			{
				...
				if (Window1Enabled && Window2Enabled)
				{
					...
					switch (PPU.ClipWindowOverlapLogic[w] ^ 1)
					{
						...
						case CLIP_XOR:
							...
							else
							{
								...
								invert = !invert;
								points[p++] = 0; /* Build an array of points (window edges) */
								...
								points[p++] = 256;
								qsort((void*) points, p, sizeof(points[0]), IntCompare); /* Sort them */
					...
					if (invert)
					{
						...
						for (b = 0; b < B; b++) /* First remove all empty bands from the list. */
						{
				...
				{
					/* Only one window enabled so no need to perform complex overlap logic... */
					if (Window1Enabled)
					{
						if (invert)
						{
				...
				if (w != 5 && pClip->Count[5])
				{
					...
					{
						/* Intersect the colour window with the bg's own clip window. */
						uint32_t i, j;
						...
						for (i = 0; i < pClip->Count[w]; i++)
						{
							for (j = 0; j < pClip->Count[5]; j++)
							{
									 ...
									 pClip->Left[j][5] <  pClip->Right[i][w]))
								{
									...
									pClip->Left[i][w]  = MATH_MAX(pClip->Left[i][w], pClip->Left[j][5]);
									pClip->Right[i][w] = MATH_MIN(pClip->Right[i][w], pClip->Right[j][5]);
									goto Clip_ok;
								}
							}
